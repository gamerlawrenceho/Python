...
def shield_update():
    ...
    shield_h = img_shield.get_height()  #計算保護罩高度
    screen.blit(img_shield, [0, bg_y - 40], [0, 0, shield_w, shield_h])


def show_gameover():
    screen.blit(img_gg, [
        bg_x / 2 - img_gg.get_width() / 2, bg_y / 2 - img_gg.get_height() / 2
    ])
...
while True:
    ...
    if gameover:
        ...
        for enemy in emy_list:  # 移動和繪製所有敵機
            ...
            if enemy.active and is_hit(ss_x, ss_y, enemy.x, enemy.y, ss_wh +
                                       enemy.wh) and ss_invincible == 0:
                ss_invincible = 40
                score -= 1
                ss_shield -= 20
            if ss_shield <= 0:
